...
class BaseDatabase():
    ...
    def __init__(self, sub_id, input_dir, work_dir, process_rt=False,
        ...
        if xnat_source or xnat_sink:
            self.pyxnat = PyxnatDatabase(
                self.xnat_config, project_id=self.xnat_project_id,
                processed_session=self.xnat_processed_session,
                overwrite=self.xnat_overwrite)
        elif cluster_source or cluster_sink:
            self.cluster = ClusterDatabase(
...
#                     rt['labels'].append(rt_session)
    ...
    def cluster_datasource(self):
        
        skip_sessions = check_cache(self.sessions, self.input_needed,
                                    self.sub_id, self.base_dir)
        
        if [x for x in self.sessions if x not in skip_sessions]:
            self.cluster.get(self.base_dir, subjects=[self.sub_id],
                             needed_scans=self.input_needed,
                             skip_sessions=skip_sessions)

    def cluster_datasink(self):
