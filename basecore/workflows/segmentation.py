...
def single_tp_tumor_segmentation(datasource, sub_id, sessions, gtv_model,
                                 tumor_model, result_dir, nipype_cache, reference,
                                 reg_workflow=None, bet_workflow=None):
    ...
    if reg_workflow is not None:
        ...
        workflow.connect(bet_workflow, 'bet.out_file', tumor_seg, 't1')
        if reference:
            workflow.connect(reg_workflow, 'regT12CT.regmat', apply_ts_tumor, 'transforms')
            workflow.connect(reg_workflow, 'regT12CT.regmat', apply_ts_gtv, 'transforms')
            workflow.connect(reg_workflow, 'regT12CT.regmat', apply_ts_tumor1, 'transforms')
    else:
        if reference:
            workflow.connect(datasource, 'regT12CT', apply_ts_tumor, 'transforms')
            workflow.connect(datasource, 'regT12CT', apply_ts_gtv, 'transforms')
            workflow.connect(datasource, 'regT12CT', apply_ts_tumor1, 'transforms')
        workflow.connect(datasource, 'ct1_preproc', mi, 'in1')
        workflow.connect(datasource, 'flair_preproc', mi, 'in2')
        workflow.connect(datasource, 'ct1_preproc', tumor_seg, 'ct1')
        workflow.connect(datasource, 't2_preproc', tumor_seg, 't2')
